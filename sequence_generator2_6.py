...
def generate_hairpin_first_half(half_length, target_entropy, full_target_state_changes, target_gc_initial,
                                entropy_tol=0.01, progress_freq=1000000):
    """
    Generates the first half of the hairpin stem (length = half_length) that meets the following:
      - Its nucleotide composition yields a Shannon entropy within tolerance of target_entropy.
      - Its composition is capable (by its raw state-change range) of achieving the target state-change.
      - The sequence's GC Inicial run is exactly target_gc_initial.
    
    Since the full hairpin state-change (from the feature file) includes:
        first-half state-change + junction (always present) + second-half state-change,
    and assuming symmetry (first-half state-change = second-half state-change),
    the candidate first half must achieve:
        first_half_target_state_change = (full_target_state_changes - 1) // 2

    To reduce search space further:
      1. Use an allowed list per position based on target_gc_initial:
         - Positions < target_gc_initial: only allow ['C', 'G'].
         - If half_length > target_gc_initial then position == target_gc_initial: allow only ['A', 'U'].
         - For subsequent positions: allow all nucleotides.
      2. Precompute valid compositions (by entropy and first-half state-change).
    
    Prints progress every 'progress_freq' candidates.
    
    Returns: (candidate_string, total candidates tested)
    """
    nucleotides = ['A', 'C', 'G', 'U']
    # Adjust the target state-change to apply to the first half.
    first_half_target_state_change = (full_target_state_changes - 1) // 2

    # Construct allowed lists per position.
    allowed = []
    ...
    # --- New: pre-filter by having enough G+C for the initial run ---
    valid_gc_comps = {
        comp
        for comp in gen_compositions(half_length)
    ...
    for candidate in itertools.product(*allowed):
        ...
        # Compute composition as tuple (n_A, n_C, n_G, n_U).
        comp = tuple(candidate_list.count(nt) for nt in nucleotides)
        if comp not in valid_comps:
            continue
        
        # Check ordering-dependent features.
        if state_change_count(candidate_list) != first_half_target_state_change:
            continue
        ...
        return candidate_str, candidate_count
...
def optimize_terminator(target_strength_val, max_hp_val, threshold_val, hairpin_tol_val, seed_val):
    import pandas as pd
    import numpy as np
    from scipy.optimize import differential_evolution
    ...
    try:
        ...
        desired_threshold = threshold_val
        best_so_far       = {"cost": np.inf, "x": None}
        ...
        snapped = snap_to_valid_candidate(
        ...
        df_final  = convert_columns_to_int(df_scaled, [
        ...
        found_u, _ = generate_u_tract_by_distribution(
